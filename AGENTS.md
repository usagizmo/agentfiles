...
- `./test/` は `bun test` の gate（起動の flag は `.githooks/pre-commit` が SSOT）。skills の `scripts/` `assets/`・`.githooks/`・共通 `AGENTS.md`・tracked ファイルのコメントを検査する。agent へは**投影しない**（`lib/inventory.sh` に載せない）
