...
- アドバイザーの起動は `consult` skill の単一実体（`references/advisors.md` + `scripts/consult-backend.sh` + `advisors.sh` / `advisors-tmux.sh` + `tmux-session.sh` + `advisors.ts`）にし、harness ごとの上書きも project **差分も置かない**。backend は `CONSULT_BACKEND` で明示（silent fallback 禁止）。実装役の dispatch は `dispatch` skill + `consult/scripts/dispatch-backend.sh` / `worker-tmux.sh`（`DISPATCH_BACKEND`。permission 差は `consult/references/dispatch.md`）。session primitive は `agents/shared/tmux-session.sh`。kind と起動 args の表とその解釈は `agents/shared/roster.toml` / `roster.ts`（`advisors` = consult の相談役、`resolve` = resolve の実装役）
