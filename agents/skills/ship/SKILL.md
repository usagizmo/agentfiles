...
**意図の確認だけは variant を問わず必ず見る**。人が 1 件を直接頼んだ課題はキュー管理を通らずにここへ来るので、**ここが最後の砦になる。**
...
3. **この PR の head を base にしている open PR があれば `gh pr edit <子> --base "$(gh repo view --json defaultBranchRef --jq .defaultBranchRef.name)"` で張り替える。** head ブランチを消さない運用では GitHub が付け替えないので、放置すると子が merge できないまま残る
4. `sh <skills root>/ship/scripts/sync-local-default.sh` でローカル default を最新化する。**path は skill 側の実体を指す。** マージした PR と変更の要点を報告する
