...
"worktree list")
  if [ -n "${HERDR_WT_PATH:-}" ] && [ -d "$HERDR_WT_PATH" ]; then
    printf '{"result":{"source":{"source_workspace_id":"%s"},"worktrees":[{"path":"%s","is_linked_worktree":true,"open_workspace_id":"%s"}]}}\n' \
      "${HERDR_WT_SOURCE:-}" "$HERDR_WT_PATH" "${HERDR_WT_WS:-}"
  else
...
# --- 自分が居る workspace は、pane を退避先へ移してから閉じる ---
: >"$HERDR_CLOSED"
sandbox herdr-self
run "$TMP/herdr-self/work" "$TMP/herdr-self" \
  HERDR_WT_PATH="$TMP/herdr-self/wt" HERDR_WT_WS=wZZ HERDR_WT_SOURCE=w1 \
  HERDR_WORKSPACE_ID=wZZ HERDR_PANE_ID=wZZ:p1
herdr_self_code=$?
...
if grep -q 'pane move wZZ:p1 --workspace w1 --new-tab' "$HERDR_MOVED"; then
  ok
else
  fail "herdr-self: pane を退避していない: $(cat "$HERDR_MOVED")"
fi
if grep -q '^wZZ$' "$HERDR_CLOSED"; then
  ok
else
  fail "herdr-self: 退避したのに workspace を閉じていない"
fi
...
run "$TMP/herdr-stuck/work" "$TMP/herdr-stuck" \
  HERDR_WT_PATH="$TMP/herdr-stuck/wt" HERDR_WT_WS=wZZ HERDR_WT_SOURCE= \
  HERDR_WORKSPACE_ID=wZZ HERDR_PANE_ID=wZZ:p1
herdr_stuck_code=$?
if [ "$herdr_stuck_code" -ne 0 ]; then
  ok
else
  fail "herdr-stuck: 積み残しごと成功で終えた"
fi
if [ ! -s "$HERDR_CLOSED" ] && [ ! -s "$HERDR_MOVED" ]; then
  ok
else
  fail "herdr-stuck: 退避先が無いのに動いた"
fi
