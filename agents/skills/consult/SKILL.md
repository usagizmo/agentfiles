...
`start` が失敗して run dir が無いとき、または agent が終端したとき（`verify` が「生きている advisor が無い」）は、自分の判断だけで 1 巡して終え、**未レビューとして報告する**（レビュー合格ではない）。`不通` は終端（次手は `references/advisors.md` の失敗時）。
