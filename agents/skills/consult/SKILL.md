...
**終了ゲート（厳守）**: 「指摘なし」以外・必須が残る・「十分」「後で」「N 巡で打ち切り」なら、`close` / `finish` 続行 / Draft·Ready PR / レビュー合格の報告を**してはならない**。直して `ask` するか、上の 2 連続エスカレートだけ。`close` 後に直したら再 `ask` するまで未終了。レビュー合格を報告するときは `verify` の出力（run dir・巡・判定）をそのまま貼る。
