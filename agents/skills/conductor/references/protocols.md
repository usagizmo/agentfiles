...
後発に休止を伝えたことを外部化する。conductor が書き、conductor が消す。
消す契機と、解除条件を足してよいかの判定は本文「action の優先順」が SSOT。
ここは書式だけ —— **述語の実体を写すと片方だけ古くなる。**
...
1. **PR にセッションまとめが載っているか確認し、`積み残し・懸念` を人へ渡す**（`resolve` は着地前に書く規約）。
   **PR が無い課題は Issue のコメントを見る** —— PR まで進まずに終えたときの置き場がそちらなので、
   見ないと「どこにも無い」と誤判定して片付けが毎 tick 空振りする
...
4. 片付けたら、人待ちの記録が `waiting` のままなら `cleared` にする（`wait-record.md` の例外行）。
   ...
   **渡しの記録が残っていれば同じ手順で消す** —— 残すと merge の枠が空かない

**存在確認だけで済ませない**。まとめは「消えていないこと」を確かめる対象ではなく、人が読んで次の
アクションを決める材料。`積み残し・懸念` は 5 項目（何が / 影響範囲 / 放置すると / 判断材料 / 推奨）で
書かれているので、そのまま状況ボードと応答へ出す。

**「対応不要」と書いてあっても落とさない**。それは書いた側の推奨であって、人の判断ではない。
実際に、非目標だと誤って整理された改名がまとめの `反映しなかったもの` に埋もれ、人が拾って
Issue になった事例がある。判断の材料を渡すのが片付ける側の責務で、取捨は人が決める。
...
条件（`capacity` / セッション / 人待ちの記録）を全部外して二度と選ばれなくなり、報告だけが消える。
毎 tick 報告が出続けるのが正しい。
...
**人待ち中と休止中は伝えない**。人の回答を待っている最中に別の指示を送ると質問が流れ、休止中に送ると
休止が破れて（`稼働中` に戻って）促し直しの失敗が積まれる。休止した課題は再開時に再 plan する。
`cleared` になって `待機` に落ちた時点で、**枠を渡すより先に**伝える（優先順が既にその形）。

「本文の変更」も同じ扱い。届け先が居ない・人待ち中は伝えず、起こし直しを先に通す。
収束条件はその action の発火条件が偽になること（条件と例外の解除は本文「1 tick」が SSOT）。
