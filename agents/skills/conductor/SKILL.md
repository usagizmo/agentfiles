...
`Conflict` はラダーで解決できないもの**だけ**。「2 つの行に当たった」は含ま**ない**。一覧は `src/types.ts` の `CONFLICT_REASONS`。

- 観測できない
- 証跡が矛盾している
- `ledger` が下のラダーで 5 に落ちた
- group の中で終端と非終端が混在している
- 着地面が解決できない。当てるのはその面を着地面に持つ課題**だけ**
- merged な PR があるのに、提出の証跡が無い。`取り下げ` に落とさ**ない**。**PR を使わない面には当てない**
- live checkout が異常。当てるのは、その面を着地面に持ち、かつその面がまだ終端していない課題**だけ**。自動で解消し**ない**。**PR で着地する面は対象外**。課題単位の終端で判定し**ない**
- Issue 契約が欠けているのに、捨てられない成果物がある（commit または dirty）
- 計画コメントが無いのに実装の証跡がある（commit または dirty）。保守的に全交差のまま保持し続ける
- 意図の確認が `pending` なのに、人待ちの記録が `cleared` か無い
...
- 人待ちの SSOT は Issue の記録であって、multiplexer の印では**ない**。**印と記録が食い違ったら記録が正**（印だけがあるときは `Conflict`）
