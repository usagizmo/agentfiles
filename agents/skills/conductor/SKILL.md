...
**`待機` / `休止` に落ちた課題はすべて「枠を渡す」の受け手**。既に write を保持しているかどうかで絞らない —— `実装中` で write を保持したまま `待機` に落ちた形（人待ちから復帰した・渡された直後にターンが終わった）を外すと、実装の途中で止まったセッションを起こす経路が 1 つも無くなる。他の action はどれも当たらない —— 「解決を起こし直す」はセッションが無いときの経路、「checks を引き直させる」「意図の確認を促す」は `提出中` / `着地待ち` が前提、「差し戻す」の 4 事象のどれでもない。資源キーの交差で対象集合自身を除くのは、この形が受け手になるため。
