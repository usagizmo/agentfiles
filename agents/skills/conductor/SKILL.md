...
| いつ                                         | どれ                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 記録を読んで判定するとき（正規化・発火条件） | `references/wait-record.md`（人待ち）・`references/same-branch.md`（宣言の読み取り・帰属・代表）・`references/ready-record.md`（在庫の鮮度）・`references/body-digest.md`（本文と計画の突き合わせ）・`references/intent-record.md`（意図の確認）・`references/integration-record.md`（merge の枠）・`references/exhaust-record.md`（実行器の枯渇）・`references/protocols.md`（休止と失敗の記録の書式） |
...
| 実行器の枯渇             | 固定 marker 付きの Issue コメント（`references/exhaust-record.md`）                                                                             |
...
| 2   | **片付ける**                 | `progress` が `着地済み` / `取り下げ`、**かつ**（`capacity` が `あり` / `prunable` または セッションが残っている または 人待ちの記録が `waiting` または 渡しの記録が残っている または 枯渇の記録が残っている または 周回の記録が残っている）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 成果を確認してから実体を消し、記録を閉じる（手順は `references/protocols.md`。**成果の確認を飛ばさない**）                                                                                                                                                                                                          |
| 3   | **計画セッションを片付ける** | `refine-<番号>` のセッションがあり、`runtime` が `人待ち` でない（**枯渇の記録が `exhausted` なら `人待ち` でも当たる** —— 下記）、`ledger` が `未計画` でないか**そのセッションが動いていない**。かつ `ledger` が `未計画` なら失敗の記録の `count` が上限に達していない                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **`done` なら入れ物を閉じ、`idle` なら `retired-refine-<番号>` へ rename する**（どちらも計画枠を空ける。下記）。**枯渇の記録が `exhausted` なら `idle` でも閉じる**（下記）。`ledger` が `未計画` のまま片付けるなら失敗の記録を進める（下記）                                                                     |
...
| 9   | **解決を起こし直す**         | `progress` が `準備中`〜`着地待ち` かつ、**① セッションが無い**（`runtime` が `無し` / `人待ち` / `休止`。**`休止` でも起こす** —— 起こされた側は記録を読んで書き始めないので、実体が観測できるようになるだけ）**か、② 枯渇の記録が `exhausted` で、そのセッションが動いていない**（`稼働中` なら選ばない —— 記録を書いてから応答を終えるまでの窓がある。`人待ち` でも入れ替える —— 下記）。かつ対象集合の台帳が揃っている（揃っていなければ「台帳を進める」が先）。checkout が無く、作ると容量が目安を超えるなら選ばない —— ただしその課題が論理 lease を保持しているなら、目安を超えても起こす（新規の claim ではなく保持者の回収。控えると、保持者が容量の解放を待ち、容量を持つ課題が保持者の write 解放を待つ循環になる）        | checkout があるなら入れ物だけ。無いなら worktree を作ってから起こす。**② は入れ替えなので、先に旧セッションを止める**（下記）                                                                                                                                                                                       |
...
**枯渇の記録が `exhausted` のまま実行器を起こさない**。セッションを起こす action すべて（claim する・解決を起こし直す・計画を起こす・計画を起こし直す）に掛かる不変条件で、起こす前に `cleared` にして書けたことを確かめる。**確かめられなければ起こさない** —— 起こされた側は `exhausted` を読むと逸脱として報告してから自分で消す契約なので（`references/exhaust-record.md`）、消し損ねたまま起こすと消す責任が黙って向こうへ移る。当人の自己解除は往復を止める回復経路であって、こちらの手順を省いてよい理由ではない。

**その前に、同じ対象の無効な人待ちの記録を閉じる**（判定は `references/wait-record.md`「記録が有効かどうか」）。枯渇の記録はその判定の証跡なので、先に消すと人待ちの記録が証跡を失って壊れた記録に変わり、次の観測で `Conflict` に落ちる —— 塞いだはずの経路がそのまま戻る。

**枯渇は実行器の入れ替えであって、枠の渡し直しではない**。渡し直しても同じ尽きた実行器なので進まない。セッションが残っていれば、先に止めて消えたことを観測してから上の手順に入る。worktree と未コミットは残す。止められなければ `Conflict`。

**`人待ち` でも入れ替える**。尽きた実行器は人の答えを処理できないので、待つあいだ実行器が要らないと読むと出口が無くなる —— 人待ちの記録を `cleared` にできるのは答えを解釈する実行器だけで、それが尽きている。起こし直された側が記録を読んで、まだ答えが無ければ質問を出し直し、返っていれば解釈する（`references/wait-record.md`「起こし直されたら」）。**人待ちの記録はそのまま残す** —— 消すのは答えを解釈した当人。**計画側も同じ** —— 枯渇の記録があれば「計画セッションを片付ける」は `人待ち` でも閉じ、次の tick で起こす側が `cleared` にして起こす。閉じないとセッションが残るので起こし直しにも当たらず、計画枠を焼いたまま滞留する。

**枯渇そのものは失敗として数えない**（実行器の寿命であって、操作の失敗ではない）。操作の失敗として数えるのは入れ替えが `稼働中` に届かなかったときで、成果ゼロの周は周回の記録が別に数える。

**`cleared` にすることと、コメントごと消すことを混ぜない**（主体は `references/exhaust-record.md` が SSOT）。ここで足すのは 1 つだけ —— **「計画セッションを片付ける」は記録に触らない。閉じるだけ。** 消させると、消せたが閉じられなかった半端が残り、次の tick からは枯渇由来だと分からなくなる。

`refine` の枯渇では「計画セッションを片付ける」が閉じ、**`count` は進めない**（`ledger` が `未計画` のまま閉じる形を数える規則の例外。述語は「枯渇の記録が `exhausted`」で、閉じても消えないので次の tick からも観測できる）。**`idle` でも rename せずに閉じる** —— rename は人が入力を書いている最中を守る規則だが、枯渇の記録はその待ちではないことの証跡そのものなので、当てると人が pane を閉じるまで再計画できなくなる。

**`待機` / `休止` に落ちた課題はすべて「枠を渡す」の受け手**。既に write を保持しているかどうかで絞らない —— `実装中` で write を保持したまま `待機` に落ちた形（人待ちから復帰した・渡された直後にターンが終わった）を外すと、実装の途中で止まったセッションを起こす経路が 1 つも無くなる。他の action はどれも当たらない —— 「解決を起こし直す」はセッションが無いか枯渇の記録があるときの経路、「checks を引き直させる」「意図の確認を促す」は `提出中` / `着地待ち` が前提、「差し戻す」の 5 事象のどれでもない。資源キーの交差で対象集合自身を除くのは、この形が受け手になるため。
...
| 待機中                  | 何を待っているか、何が空けば動くか。**`休止` もここに載せる**（何が着地すれば再開できるか。write 非保持だが未コミットは抱えている）。**枯渇は「実行器の入れ替え待ち」として分けて出す** —— 出さないと、渡し直せば進むものと区別がつかない                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 正規化レコードと容量                                                                           |
...
**表に載らない滞留を行にするのは、どれも「異常が起きた」形では現れないから**。振った作業は action にも数えないので「この tick の action」も動かず、`retired-refine-<番号>` は人が pane を閉じるまで再計画されず、起票した Issue は人が動かすまで選ばれない。行にしないと更新の契機にも掛からず、黙って止まる。どこへ置くべきかは書かない —— 優先順は製品判断で、conductor の領分ではない。

**禁じているのは順序を自分で決めることであって、並べ替えの操作ではない**。自分で決めると人が並べた順を上書きすることになり、ボードが観測材料であることが壊れる。人が順序を指定したなら実行する —— 決めたのは人なので、ボードは観測材料のまま（手段は `references/intake.md`）。
