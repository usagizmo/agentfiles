...
**人が優先順を変えたいときは、ボードを並べ替えてもらう**。指示を conductor の記憶にも永続コメントにも置かない — tick は前回の続きを仮定しないので記憶は次の tick で消え、コメントは失効条件を持てないので古い優先順が残り続ける。ボードは観測材料そのものなので、並べ替えれば次の tick から自動で効き、気が変わればまた並べ替えるだけで戻る。
