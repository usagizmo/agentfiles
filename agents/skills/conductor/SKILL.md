...
**1 件を止めるのは「差し戻し」、全体を止めるのは conductor セッション自体の停止**（「実行の制約」）。

| `Decision`      | 何をするか                                                                                                                                                                                     |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `conflicts`     | 触らずに状況ボードへ出す。**同内容は追記しない**。**1 手を選べた周でも出す**（落とすと、キューが動いているあいだ異常が誰にも見えない）                                                         |
...
`ledger` が `退避先` の課題に当てるのは、上 2 つ（片付ける・計画セッションを片付ける）と、「失効した記録を片付ける」の merge の枠の回収**だけ**。判定は Status の値だけで行う。**`Conflict` の報告は `ledger` に依らない**（ラダーの外なので）。
...
**claim の条件に交差を入れない。例外は、候補側の値に依らず結論が決まると project が宣言したときだけ** ——「既存側だけで、これから来るどの候補とも非互換だと証明できる」形。いつ置くかは project の領分、運び方は `references/issue-contract.md` が固定する。
...
- 単一の課題が証跡の矛盾で全交差保持に落ちているあいだも、宣言と同じに扱う（「計画コメントが無いのに実装の証跡がある」）
...
「claim する」の条件のうち、**容量以外の**どれかが偽であること（`src/decide.ts` の `claimStructurallyBlocked`）。

**列挙を持たないので、claim の条件が増えても追随する**。写した列挙を他の節に作らない。

**除外の SSOT はここ**。資源の空きで決まる条件を claim に足したら、この除外にも足す（自動では追随しない）。
...
**人が優先順を変えたいときは、ボードへ反映する**（規則は「状況ボード」が SSOT）。指示を conductor の記憶にも永続コメントにも置かない。
...
更新するのは**下の表のどれかの行が変わったとき**。どの行も変わっていない tick では、鮮度だけ差し替える。**条件を項目名で列挙しない。**
