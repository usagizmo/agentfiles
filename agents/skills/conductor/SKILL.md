...
| いつ                                                     | どれ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 記録とセッションを読んで判定するとき（正規化・発火条件） | `references/harness.md`（セッション状態値の意味）・`references/wait-record.md`（人待ち）・`references/same-branch.md`（宣言の読み取り・帰属・代表）・`references/landing-surface.md`（着地面の解決・面ごとの終端・統合先・live checkout）・`references/session-report.md`（提出の観測材料）・`references/ready-record.md`（在庫の鮮度）・`references/body-digest.md`（本文と計画の突き合わせ）・`references/intent-record.md`（意図の確認）・`references/integration-record.md`（merge の枠）・`references/protocols.md`（休止・失敗・周回の記録の書式） |
...
| tick の意味論を書き換えるとき                            | `references/scenarios.md`（射程の定義と期待の回帰）・`references/tick.md`（正規化と action の論証）・`references/resources.md`（資源の論証）                                                                                                                                                                                                                                                                                                                                                                                                             |

**1 行目を「action を選んだ後」に丸めない**。そこに挙げた記録とセッション状態は本文に述語を置いていないので、選ぶ前に読まないと正規化も発火条件も毎 tick 判定できない。「観測」の SSOT 表が指す先がここに揃う。
...
| **入場を止める宣言**     | 固定 marker `entry-block` のコメント（形は `references/issue-contract.md`。扱いは「入場を止める宣言」）                                                                             |
...
**tick の意味論を決める節を書き換えるときは `references/scenarios.md` を先に確認する**（射程も、期待が変わったときの扱いも、そちらが定める）。
...
| 周             | 条件                                                         |
| -------------- | ------------------------------------------------------------ |
| write を渡す   | `progress` が `準備中` / `準備済み` / `実装中` / `提出中`    |
...
| 11  | **枠を渡す**                 | `runtime` が `待機` / `休止`。**渡すのが write なら**資源キーが write 保持者（対象集合自身は除く）のどれとも交差せず、かつ入場を止める宣言を持つ課題が居るならまだ write を保持していない課題へは渡さない（「資源」。保持者への渡し直しと**宣言した課題自身への再貸出**は止めない —— 止めると、既に書いている課題が渡し直しを待ち、宣言した課題がその終端を待って両方止まる。宣言した課題自身を止めると、そちらは二度と再開できない）。**integration なら**渡しの記録を持っているか、記録がどこにも無くて次の受け手に選ばれている（選び方と宣言の扱いは「資源」）。どちらを渡すかは `progress` が決める（`着地待ち` は write を保持していないので integration、それ以外は write。保持条件は「資源」） | **write を渡すときだけ、休止の記録を先に消してから**渡して再開させる（integration では消さない —— 休止は write の交差でしか起きないので、消すと交差が残ったまま両方が書ける）。integration は、渡す前に渡しの記録を書く（記録が権限の実体。逆にすると、伝達が成功して記録が落ちたときに次の tick が別の課題へ渡す） |
| 12  | **計画を起こす**             | `ledger` が `未計画` かつ `progress` が `未着手` かつ**その番号の計画セッションが無く**（`retired-refine-<番号>` も「有る」に数える）、人待ちの記録が `waiting` でない。**かつ在庫と計画枠に空きがある**（「計画済みの在庫」と「資源」）                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `refine` を起こす                                                                                                                                                                                                                                                                                                   |
...
- **`runtime` には写さない**（`無し` として扱う）。写すと `待機` のまま「枠を渡す」の受け手になり、**人が入力を書いている最中の pane へ枠が渡る** —— 閉じずに rename する理由そのものが消える
...
**claim の条件に交差を入れない**（候補の `resourceKeys` は計画コメントにしか無く、計画は claim して prepare を走らせた後にできる）。**例外は、候補側の値に依らず結論が決まると project が宣言したときだけ** ——「既存側だけで、これから来るどの候補とも非互換だと証明できる」形。**いつ置くかは project の領分、運び方は `references/issue-contract.md` が固定する** —— 形まで project に委ねると、読む側が観測する手段を持てない。
...
**置き場は固定 marker のコメント**（`references/issue-contract.md`）。指紋に入らない形に置くと、解除しても誰も tick を起こさない —— 宣言は claim と write を止め、**在庫の鮮度の評価も止める**ので、止まったままの窓が最も長く残る。
...
**排他ラダーにする**。上から読み、最初に当たった行が勝つ（各行は上の行に当たらないことを前提にする）。生の条件は重なってよい — 重なりは順序が解決する。**上限に達した行を上に置く。**

**`ledger` の範囲は行ごとに持つ。全行共通の 1 文へ外出ししない。**

| 何が起きた           | 判定（上の行に当たらないこと）                                                                                                                                                                                                                                                                                                                                                                                                  | 戻し先       | あわせてすること                                                      |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | --------------------------------------------------------------------- |
| retry budget 切れ    | **`ledger` が `未計画` / `計画済み` / `進行中`** かつ、失敗の記録の `count` が上限に達した                                                                                                                                                                                                                                                                                                                                      | **退避先**   | **`count` の精算は「1 tick」の規則に従う**                            |
| 成果ゼロの周が続いた | **`ledger` が `未計画` / `計画済み` / `進行中`** かつ、周回の記録の `count` が上限に達し、現在の指紋が `mark` と一致し、有効な人待ちの記録が無く（判定は `references/wait-record.md`。人待ちを差し戻しに含めないという規則はここにも掛かる）、**そのセッションが動いていない**（正規化後の `runtime` では引かない —— 人待ちの記録があると生きたセッションでも `人待ち` に写るので、起こした直後の実行器を結果が出る前に落とす） | **退避先**   | **`count` の精算は「1 tick」の規則に従う**                            |
| claim だけ残っていた | **`ledger` が `計画済み` または `進行中`** かつ `progress` が `未着手` かつ **claim の記録がある**かつ Issue 契約は揃っている                                                                                                                                                                                                                                                                                                   | **計画済み** | **claim の記録を消す**（残すと claim 済みに見えて二度と選出されない） |
| 計画が無効           | **`ledger` が `計画済み` または `進行中`** かつ **Issue 契約が欠けている**かつ commit も dirty も無い                                                                                                                                                                                                                                                                                                                           | **未計画**   | **claim の記録を消し、全着地面の branch と実体を片付ける**            |
...
**「claim の痕跡」は claim の記録で引く**。`progress` が `未着手` なら claim branch は無いので（在れば `準備中`）、この形で残りうる痕跡は記録だけ。**「claim 済み」の 4 通りでは引かない**（判定は「選出」）—— 記録を持たない形まで拾うと、消す記録が無いまま Status だけ戻すことになる。**記録の無い `進行中` × `未着手` はどの行にも当たらず、`Conflict` に落ちる**（消すものが無いので、branch が生えるか人が Status を戻すまで解けない）。

**「計画が無効」を claim の記録の有無では絞らない**。計画工程が契約を埋め損ねたまま計画済みへ進めた形が拾われず、選出の条件にも弾かれて永久に沈黙する。

**「在庫が陳腐化した」に claim の除外を足さない**。「claim が構造的に止まっていない」が選出の条件「まだ claim されていない」を通じて claim 済みを既に除いている。足すと、列挙を持たないと決めてある述語の写しになり、claim の条件が増えたときに写した側だけが黙って古くなる。
...
**自分の context の残量を制約として報告しない**（ボードにも応答にも）。100% で自動圧縮され、人が明示的に圧縮することもできるので、**回り続けられるかどうかは conductor の判断に掛かっていない**。書くと、人が動かせない数字を詰まりとして読み、要らない交代や中断を検討することになる。交代の契機は `references/harness.md`「交代」が持つ。
