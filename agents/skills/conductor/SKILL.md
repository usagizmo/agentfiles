...
**判定に要る記録の読み方は `src/` が持つ**。`references/` を毎 tick 読まない —— 正規化も
発火条件も `cli.ts` が返すので、読む必要があるのは自分がやる 5 つに当たったときだけ。
...
**意味論を変えるなら、先にテストを変える**。`test/decide.test.ts` と `test/normalize.test.ts` が
期待の SSOT で、`references/scenarios.md` は同じ行 ID の解説。**表だけを直しても何も変わらない。**

**自分がやるのは 5 つだけ** —— 実行器へ渡す prompt 本文、`Conflict` の人向け説明、状況ボードの文章、
`intake` の分類、規約の穴の起票。**どれも「決まったことを人が読む形にする」仕事で、判断ではない**
（後ろ 2 つだけが判断で、そこは機械化しない）。
...
**一覧は必ず全件取る。先頭 N 件で打ち切らない**（ページングを最後まで回す。上限つきの API は上限に達したこと自体を失敗として扱う）。部分観測は禁じ手で、これは rate limit のときだけの話ではない —— 既定の `first: N` をそのまま使うのが最も踏みやすい経路で、返ってきた件数が上限ちょうどかどうかを見ない限り、欠けていることは観測に現れない。ボードの数字は人が判断に使うので、気づけない誤りになる）。
...
**述語の実体は `src/normalize.ts`、期待は `test/normalize.test.ts`**。ここに写さない —— 写した側は黙って古くなり、片方だけ直る。
...
**「空でない」を面ごとに要求しない。**要求すると、**その面に書かなかった課題が終端から締め出される**。0 commit の面は透過させ**、成果ゼロを弾く役目は課題側の「提出の証跡」が引き取る**（`references/landing-surface.md`。git の範囲で測るとどの測り方も着地の前後どちらかで壊れる）。
...
**commit が 0 の面は `着地待ち` でも透過する。終端と同じ扱いを、ここでも落とさない**。予定した面に結局書かなかったことは通常運用で起きるが、**PR を使う面に commit が無いと PR を作れない**ので、透過しないとその課題は永久に `着地待ち` へ到達せず、integration の受け手にならない。透過は commit 包含の判定にだけ効かせる。dirty まで透過させない。**書かなかった面が dirty のまま透過すると**、`着地待ち` も `着地済み` も成立し、片付けがその面の worktree ごと未コミットを消す —— しかも「揃っていない面の dirty は `実装中` が拾う」は、透過で揃っていると読まれた面には当たらない。dirty を全面の共通前提（`0` のみ。`1` も、読めなかった `-` も不可）にすれば、その面はラダーを落ちて `実装中` になり、write を保持したまま直せる。
...
**対応の実体は `src/decide.ts` の `expectedLedger`**。同上。
...
| `conflict`      | 触らずに状況ボードへ出す。**同内容は追記しない**ので、実行しても何も起きない                                                                                                                   |
...
**budget は「retry すれば成功しうる」を前提にしている。** 権限は tick では変わらないので、数えると健全な課題が budget ぶんの tick で `退避先` へ落ちる。しかも `退避先` は入場を止める宣言を外すので、環境の権限が無関係な課題の claim 可否まで動かす。
...
**action の名前と順序と発火条件の実体は `src/decide.ts` の `LADDER`、期待は `test/decide.test.ts`**。
ここに写さない —— 条件は 18 段が互いの否定を前提にしていて、写した側だけが古くなると
「上位に当たらないこと」の前提が静かに崩れる。**すること**（選んだ後の手順）は `references/protocols.md`。

**失効の判定を制御面だけで行わない。**着地面の統合先は、その面へ別の課題が着地したときに動く —— そこを見ないと、同じ面で並行している課題が古い前提のまま書き続ける**。制御面は着地面に含まれなくても必ず見る** —— project 差分と共有ファイルはそこにあり、どの課題もそれを読んで計画を立てているため。
...
**「前回」は時点ではなく、tick が action を決めるのに使った観測そのもの。** 起床側で取り直すと、tick の観測から baseline までの隙間に入った遷移が baseline に吸われ、fallback まで誰にも見えない。**窓の長さで抑えにいかない** —— 起床の契機の大半は実装セッションの commit なので、窓は遷移が起きやすい時間帯に置かれ、当たる確率は縮めた比率ほど下がらない。`--interval` / `--max` は検知の遅延の調整であって、この窓の対策ではない（既定と根拠は `references/harness.md`）。
...
- **同じ項目を、遷移を駆動しうる部分集合へ絞る**。追跡していない PR（branch 名に Issue 番号を持たないもの）の checks は、紐づく課題が無いので定義上どの `progress` も動かせない ——人が自分のブランチで CI を回すたびに起こされるだけになる。項目を落とすのではなく値を丸めているので、PR の open / close は今までどおり指紋に出る。判定を branch 名の形だけで行うこと・判定できないものは残す側（fail-open）へ倒すことは `scripts/watch.sh` が持つ
...
**保持の述語の実体は `src/resources.ts`、上限は `src/decide.ts` の `TickConfig`**。
ここに写さない —— 保持条件は「取得と解放を別に書かない」形で書かれていて、写すと片方だけが直る。
...
**判定の実体は `src/resources.ts` の `intersect`**。3 値で、`unknown` は `incompatible` として扱う。
...
**5 事象の判定と戻し先の実体は `src/decide.ts` の `revertTarget`、期待は `test/decide.test.ts`**。
ここに写さない —— 排他ラダーなので、写した側だけが古くなると行の順序が意味を失う。
...
**数えるのは、解ければ実際に動き出すものだけ**。`ledger` が `退避先` の課題は人が動かすまでどの工程も起きないので、依存を解いても動かない —— 数えると詰まりでないものを詰まりとして扱い、ボード順を飛び越えて選ぶ。人が並べた順を壊すので、目的と逆に働く。
...
| **人がやること**        | **先頭に束ねる**。述語は 「人が動かすまで、どの工程も起きないもの」全部 で、件数と何を答えれば / 何を動かせば進むかを書く。人は上から読むので、散らすと拾われない。行名で列挙しない —— 列挙すると、後から足した滞留（`retired-refine-<番号>`・入場を止める宣言）が下の行にだけ載り、人には計画枠の数字しか見えない                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 下の各行の導出                                                                                 |
| **この tick の action** | 何を選び、**なぜそれを選んだか**。数字の差分だけでは読み取れない（在庫が 6 → 1 に減った理由は、これが無いと Issue コメントを開くまで分からない）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 実行した action                                                                                |
| 稼働中                  | Issue 番号・`progress`・保持している資源・`runtime`。**branch を持つものは、あわせてその実体を着地面ごとに**（面の名前 / 未コミット数 / 統合先へ入っていない commit 数 / 統合先から何 commit 遅れているか。計画セッションは branch を持たないので出さない）。**面を畳んで合計にしない** —— どの面が遅れているかが消え、追随の相手が分からなくなる。実体を出すのは、`progress` が意図であって実体ではないからで、出さないと失われうる作業量と追随コストがどこにも現れない。                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 正規化レコードと worktree の git 状態                                                          |
