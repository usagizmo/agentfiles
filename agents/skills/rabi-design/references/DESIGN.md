...
**文書**（見積・譜面・スライド・配布する PDF）は `rabi-components.css` を読まない。文書の部品は `rabi.css` の `.rabi-heading` と `.rabi-table` だけで、表は `.rabi-table` を必ず使う。

ナビと操作と状態を持つ面は文書では**ない**。web の docs はここで言う文書に当たらず、`rabi-components.css` を読む。長文は `.rabi-prose` で組む。
