...
表の文字に従わ**ない**のは、見出しの段と、帯（ステータスバー・節の帯・面の頭）だけ —— どちらも丈に依らない。

**上の丈の表は `bun test` の gate が正規表現で読む入力**。列の順とバッククォートを変え**ない**。gate が CSS と突き合わせるのは文字と左右の余白の 2 列で、図の列は期待値との自己一致だけを見る。
