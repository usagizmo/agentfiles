...
**hover に時間を持たせない**。触れた瞬間に変える。時間を持つのは、位置か向きが変わるものだけ（スイッチのつまみ・開閉の三角）。速さは `--rabi-motion` の 1 つ**だけ**で、段を作ら**ない**。繰り返す動きを持てるのは待ちの点**だけ**。`prefers-reduced-motion` では遷移も繰り返しも消す。
...
帯の通り道は左右の余白の**外**に取り、**全行**で空ける。選んだ行だけ空けると字の左端がずれ、余白から削ると字と帯の距離が丈の表より詰まる。
...
等幅を名乗れるのはコードと打鍵の印、表の数値のセル、機械が出す値（日付・識別子・ログ）**だけ**。列見出しは字なので本文の書体のまま。サイズは部品が持つ。載る面から継ぐのは表の数値のセル**だけ**。どの部品がそれに当たるかは `../assets/rabi-components.css`。
...
表の左右の余白に従わ**ない**のは、下罫だけの入力・select・textarea —— 箱では**ない**ので余白を持たず、字の左端は容器が 1 か所で持つ。丈と文字の段には従う。
...
- 容器に `border: 1px solid divider` と `rounded: lg` と `overflow: hidden` を付ける。中の罫が外周より濃いと格子が浮くので、内側は `line`
...
半径は 5 値。段の間の値を作ら**ない**。1 つのつまみ（`--rabi-r-unit`、SSOT は `../assets/rabi.css`）が導くのは `sm` `md` `lg` の 3 値で、`none` と `full` は従わ**ない**。いまつまみは 0 なので `sm` `md` `lg` は同値。

**段は要素の大きさで選ぶ**。半径は大きさに対する比で読まれるので、同じ値を印と部品へ使わ**ない**。

| 値     | 何に付くか                                                           |
| ------ | -------------------------------------------------------------------- |
| 0      | 既定。セル・行・帯・区切り・文書と、下罫だけの入力・select・textarea |
| `sm`   | 印と、文字丈の小さな面 —— チェック・キー・色の印・会話の文・コード   |
| `md`   | 部品と、浮く操作面 —— ボタン・チップ・ドロップダウン・セグメント     |
| `lg`   | 枠になる面 —— カード・パネル・ダイアログ・結合グリッド・一覧         |
| `full` | 円形と丸端の部品 —— ラジオ・点（ステータス・窓）・バッジ             |
...
- 面の境は罫が作る。浮く面も地に置く面も外周は `divider` で、影を落とさ**ない**（「Elevation & Depth」）
...
- スイッチ: 入は軌道を赤ベタにしてつまみを反転する。切は地を持たない枠。位置**だけ**で分け**ない**
