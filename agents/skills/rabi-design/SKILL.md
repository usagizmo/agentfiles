...
1. `assets/DESIGN.md` —— 仕様の全文。front matter が機械可読なトークン、本文が適用の規則
...
front matter だけが持つ値は `assets/DESIGN.md` を直接直す。どの path がそれに当たるかは `scripts/gen-tokens.ts` の `ownedByFrontMatter`。
