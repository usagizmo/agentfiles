...
面・罫線・影はそれぞれ 3 段・3 種・3 段を持つ。段の間の値を作ら**ない**。

罫線は乗る面で選ぶ。面の上・地の上・赤の面の上で別のトークンを使う。
...
赤を出してよいのは次の 4 役だけ。どれでもない場所では黒・グレー・太字で表す。
...
| 強調   | 本文中の一語                       | 1 節に 1 つ   |
| リンク | 本文・docs のリンク                | 制限しない    |

`.rabi-heading` の下罫と `.rabi-table` の合計行は既定の強調。量に数えない。

新しい部品は「どの役か」で決める。装飾・分類・格上げには使わ**ない**（タグの色分け・グラフの系列色・見出しを目立たせる用途）。

4 役に収まらない赤が要るときは、赤を足すのではなく状態の設計を疑う。

リンクは色だけで示さ**ない**。下線を必ず添える。

面に載せる赤い文字は `--rabi-accent-text`、ベタ・枠・点・下罫は `--rabi-accent`。
...
| 選択   | `outline: 2px solid var(--rabi-ink)`。赤の面の上だけ `--rabi-accent`                                      |
| 完了   | `--rabi-accent` のベタ + `--rabi-on-accent` の印                                                          |
| 警告   | 面を `--rabi-wash`、文字を `--rabi-accent-text`。入力欄は枠を 2px の `--rabi-accent` にして説明文を添える |
| 無効   | 地を `--rabi-paper-2`、文字を `--rabi-faint`、輪郭を `--rabi-line` へ                                     |

選択で地を赤にし**ない**。

focus・選択と、エラー・警告は形で分ける —— 前者は部品の外側の `outline`、後者は枠そのものを 2px にする。
...
- 0: パネル・カード・表・区切り・リスト行・入力枠・チェックボックス。一覧に無いものもここ
- `--rabi-r-round`: ラジオ・ボタン・チップ・ステータスの点

チェックボックスを丸にし**ない**。checked の印はチェックで、丸ではない。

チェックボックス・ラジオは `appearance: none` で組む。ネイティブのままでは半径が効かない。
