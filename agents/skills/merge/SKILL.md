...
`--ff-only` に**メッセージを付けない**。`--no-ff` のメッセージは直前コミットの言語・スタイルに合わせ、gitmoji を 1 つ選ぶ（`references/gitmoji.md`）。タイトルを `Merge branch '...'` にしない。

## 衝突

方向を問わず、統合先の HEAD が対象ブランチの祖先なら、どちらの形も衝突しない。**祖先でない**なら merge せず、自分の worktree で rebase してから出直す。
...
統合先が upstream より先行していることは、**この検査に当たらない**。switch しなかったあとも、この検査は同じ。dirty を checkout / stash / reset で解消する経路は足さない。
