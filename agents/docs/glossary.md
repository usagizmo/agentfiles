...
| `capacity` | `無し` / `あり` / `prunable`（孤児 workspace。述語は `conductor/references/harness.md`） |
...
| group   | `present` は記録の `members`。`absent` は本文の推移閉包。`unobservable` / `invalid` は単独                                                                                                                                                                                                                         | `agents/shared/queue/same-branch.md` |
