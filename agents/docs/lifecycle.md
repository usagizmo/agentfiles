...
規約の本体は `conductor` / `refine` / `resolve` の各 `SKILL.md`。ここは導出した図。**知らない語が出てきたら** [`glossary.md`](glossary.md)。
...
**判定は上から読んで最初に当たった行**（終端が最上段）。順序は恣意ではなく包含関係で、終端を最上段に置く理由は `conductor/SKILL.md` の `progress` の節。

**計画セッション（`refine`）は `progress` に現れない**。成果物は進んでいないので `未着手` のままで、セッションの有無は `runtime` 側の話。

**複数の行に当たるのは正常**。`Conflict`（判断がつかない状態。該当条件は `conductor/SKILL.md`）はどの状態からも起こりうる。遷移せず、状況ボードへ出して止まる。`取り下げ` も同じくどの状態からも起こりうる（Issue が closed になれば成立する）。図は代表的な 3 本だけを描いている。

**`ledger` が期待より「手前」なら `Conflict` にしない** — 前進で直せる（claim 直後に Status 更新が失敗した `準備中 × 計画済み` がこれ）。
...
stateDiagram-v2
    ...
    state "runtime" as R {
        ...
        稼働中 --> 無し: セッションが死ぬ
        待機 --> 無し: セッションが死ぬ
        待機 --> 無し: 枯渇の記録あり → 入れ替えのため止める
    }
```

**枯渇は `runtime` の値を増やさない**。区別は記録の有無が持つ。値にすると write の保持条件・枠を渡す経路・起こし直す経路を同時に書き換えることになるが、実際に変わるのは「渡し直すのか入れ替えるのか」の 1 点だけ。**有効な人待ちの最中でも実行器は入れ替える**。ただし `runtime` は `人待ち` のまま —— 記録が `waiting` である限りセッションの有無を問わないので、止めても起こし直しても値は動かない（図に辺が無いのはこのため）。以下同じ趣旨で —— 尽きた実行器は答えを処理できないので、`人待ち` のまま止めて起こし直し、人待ちの記録は残したまま新しい実行器が読み直す。

**`人待ち` はセッションの有無を問わない**。記録が `waiting` である限り `人待ち`。だから人待ち中にセッションが死んでも値は割れず、`Conflict` にならない。起こし直された側は起動時に記録を読み、まだ答えが無ければ質問を出し直し、答えが返っていれば解釈して `cleared` にする。

**`待機` は「記録が無い」と「`cleared`」の両方を含む**。片方だけにすると、答えを解釈した直後の「動いていない」がどの値にも当たらず、人待ちからの復帰が止まる。

**`休止` は記録だけでは成立しない** —— 記録があり、かつ止まっていること。記録を書いた直後はまだ安全な地点へ向かって書いているので `稼働中` のままで、write も保持したまま。ここを丸めると、書いている最中の課題から枠を取り上げて交差する 2 つが同時に書く。

**`人待ち` も `休止` も、セッションが死んでも値が割れない**（記録が残るため）。どちらも起こし直しの対象で、起こされた側は起動時に記録を読んで、書き始めずに待つ。

**`人待ち` から直接 `稼働中` へ戻らない**。答えが返っても write は別の課題へ回っている可能性があるので、`待機` を経由して渡し直しを待つ。

**セッションが死んでも `progress` は動かない**。だから `runtime` を `無し` から起こし直すだけで続きから再開できる（実装は worktree に、計画と人待ちは Issue に残っている）。唯一の例外は commit 前に worktree ごと失われたときで、そのときは実装自体が無いので`準備済み` へ戻るのが正しい。
