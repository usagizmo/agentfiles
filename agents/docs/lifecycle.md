...
**複数の行に当たるのは正常**。`Conflict`（判断がつかない状態。該当条件は `conductor/SKILL.md`）はどの状態からも起こりうる。遷移せず、状況ボードへ出して止まる。`取り下げ` も同じくどの状態からも起こりうるが、**Issue が closed であることだけでは成立しない** —— 終端した面と終端していない面が混在していたら `取り下げ` にしない（片付けが未着地の成果ごと消すため）。図は代表的な数本だけを描いている。
