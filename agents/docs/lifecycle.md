...
**複数の行に当たるのは正常**（上から読んで先に当たった方が勝つ、という決め方なので）。`Conflict`（判断がつかない状態。該当条件は `conductor/SKILL.md`）はどの状態からも起こりうる。遷移せず、状況ボードへ出して止まる。`取り下げ` も同じくどの状態からも起こりうる（Issue が closed になれば成立する）。図は代表的な 3 本だけを描いている。
