...
見るのは worktree の中身（dirty かどうか）だけ。
...
**判定は上から読んで最初に当たった行**（終端が最上段）。順序は恣意ではなく包含関係で、
終端を最上段に置く理由は `conductor/SKILL.md` の `progress` の節。
...
遷移せず、状況ボードへ出して止まる。`取り下げ` も同じくどの状態からも起こりうる（Issue が
closed になれば成立する）。図は代表的な 3 本だけを描いている。
...
**`休止` は記録だけでは成立しない** —— 記録があり、かつ止まっていること。記録を書いた直後は
まだ安全な地点へ向かって書いているので `稼働中` のままで、write も保持したまま。ここを丸めると、
書いている最中の課題から枠を取り上げて交差する 2 つが同時に書く。

**`人待ち` も `休止` も、セッションが死んでも値が割れない**（記録が残るため）。どちらも
起こし直しの対象で、起こされた側は起動時に記録を読んで、書き始めずに待つ。
