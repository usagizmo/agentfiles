# ファイル構造と依存

規約の本体は各 `SKILL.md` と [`../../AGENTS.md`](../../AGENTS.md)（置き場所の判断と配線）。ここは実体の対応表。

## ディレクトリの役割

| ディレクトリ                | モデルの扱い                                     | 投影先                        |
| --------------------------- | ------------------------------------------------ | ----------------------------- |
| `skills/<name>/SKILL.md`    | 発動時に**常に読む**                             | `~/.agents/skills/`           |
| `skills/<name>/references/` | **読む**（必要になったときだけ）                 | 同上                          |
| `skills/<name>/scripts/`    | **実行する**                                     | 同上                          |
| `skills/<name>/assets/`     | **成果物に使う**                                 | 同上                          |
| `AGENTS.md`                 | **常時読み込まれる**                             | `~/.agents/AGENTS.md` ほか    |
| `shared/`                   | **読む / 実行する**（skill から symlink 経由で） | skills の一部として投影される |
| `docs/`                     | **読まない**（人が読む）                         | 投影しない                    |

## skill 間の参照

| 何        | 正                                                                       | 含むもの       |
| --------- | ------------------------------------------------------------------------ | -------------- |
| 層の rank | [`../skills/docs/scripts/layers.tsv`](../skills/docs/scripts/layers.tsv) | leaf 以外      |
| 層表の名  | 所有集合（述語は `../skills/docs/scripts/audit-skills.sh`）              | 所有する skill |
| この図    | 起動・依存の辺を持つ skill（全一覧は `README.md` の層構造）              | 辺を持つ skill |

```mermaid
flowchart LR
    subgraph orch[orchestrator]
        CO[conductor]
    end
    subgraph flow["work-item flow"]
        RF[refine]
        RS[resolve]
    end
    subgraph sub[subflow]
        FI[finish]
    end
    subgraph leaf[leaf]
        CS[consult]
        ZB[zero-base-loop]
        TD[tidy]
        DC[docs]
        CM[commit]
        PR[pr]
        SH[ship]
        ME[merge]
        IS[issue]
    end

    CO --> RF
    CO --> RS
    CO -.->|live checkout の着地検査に依存| ME
    RS -.->|cwd の木しか見ないことに依存| CM
    RF --> CS
    RS --> CS
    RS --> FI
    RS --> IS
    RS --> PR
    RS --> SH
    RS --> ME
    FI --> ZB
    FI --> TD
    FI --> DC
    FI --> CM
```

実線は起動、点線は挙動への依存。`conductor` は `merge` の着地検査を参照し、起動はしない。

`conductor` が multiplexer の CLI を参照する箇所は **`references/harness.md` に隔離**してあり、本体はそれ以外の場所で multiplexer を知らない。

## 共有の実体

どの skill がどの共有実体を張っているか。**置く条件と張り方の規則は [`../../AGENTS.md`](../../AGENTS.md) が SSOT**。ここには写さない。

```mermaid
flowchart LR
    subgraph orch["orchestrator"]
        CO[conductor]
    end
    subgraph flow["work-item flow"]
        RF[refine]
        RS[resolve]
    end
    subgraph leaf["leaf"]
        CS[consult]
        ZB[zero-base-loop]
        TD[tidy]
        DC[docs]
        CM[commit]
        IS[issue]
        ME[merge]
        PR[pr]
        SH[ship]
    end
    subgraph shared["agents/shared/ — 普遍"]
        RC["review-contract.md<br/><small>レビュー委譲の契約</small>"]
        AD["advisors.md<br/><small>アドバイザー起動表</small>"]
        RT["roster.toml<br/><small>アドバイザー候補と実行器</small>"]
        RTS["roster.ts<br/><small>roster.toml の parse</small>"]
        AM["advisors.ts<br/><small>選出・検証</small>"]
        AS["advisors.sh<br/><small>起動・回収の実行</small>"]
        GM["gitmoji.md<br/><small>gitmoji 一覧</small>"]
        SD["sync-local-default.sh<br/><small>ローカル default の ff</small>"]
        EI["ensure-integration-ref.sh<br/><small>統合先 ref の ensure</small>"]
    end
    subgraph sharedq["agents/shared/queue/ — キュー機構専用"]
        SB["same-branch.md<br/><small>1 本で直す宣言・group</small>"]
        IC["issue-contract.md<br/><small>Issue 契約の見出しと入場を止める宣言</small>"]
        WR["wait-record.md<br/><small>人待ちの記録</small>"]
        RR["ready-record.md<br/><small>在庫の鮮度の記録</small>"]
        BD["body-digest.md<br/><small>Issue 本文の digest</small>"]
        AF["score.md<br/><small>譜面の条件</small>"]
        IR["integration-record.md<br/><small>merge の枠の記録</small>"]
        IT["intent-record.md<br/><small>意図の確認の記録</small>"]
        LS["landing-surface.md<br/><small>着地面の意味論</small>"]
        SR["session-report.md<br/><small>セッションまとめ</small>"]
        SL["standalone-line.ts<br/><small>固定 marker の単独行</small>"]
        MU["marker-upsert.md<br/><small>記録コメントを 1 つに保つ手順</small>"]
    end

    RF --> AF
    RS --> AF
    CO --> AF
    CO --> SB
    CO --> IC
    RF --> IC
    CO --> MU
    RF --> MU
    RS --> MU
    CO --> WR
    CO --> RR
    CO --> BD
    RF --> SB
    RF --> WR
    RF --> RR
    RS --> RR
    ME --> GM
    CO --> EI
    ME --> EI
    RF --> BD
    RS --> SB
    RS --> WR
    RS --> BD
    CO --> IR
    RS --> IR
    CO --> IT
    RS --> IT
    RF --> IT
    CO --> LS
    RF --> LS
    RS --> LS
    CO --> SR
    RS --> SR
    RF --> SR
    CO --> SL
    RS --> SL
    CS --> AD
    CS --> AS
    ZB --> AD
    ZB --> AS
    AS --> RT
    AS --> AM
    AM --> RTS
    CO --> RT
    CO --> RTS
    TD --> RC
    DC --> RC
    CM --> GM
    IS --> GM
    PR --> GM
    SH --> GM
    PR --> SD
    SH --> SD
```

掲載条件は所有判定だけ。`*.test.*` は本体の行へ畳む。

skill 固有の `references/`:

| skill         | 実体                                                                                                                                | 何を持つか                                                                                                                                                      |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `conductor`   | `harness.md` / `roster.toml`（`shared/` への symlink） / `protocols.md` / `intake.md` / `tick.md` / `resources.md` / `scenarios.md` | multiplexer 差分 / 工程ごとの実行器 / 選んだ後の手順 / 人が渡してきたものの扱い / 正規化と action の論証 / 資源の論証 / **tick の意味論を固定する代表シナリオ** |
| `resolve`     | `replan.md` / `intent.md` / `judgment.md` / `scope.md` / `written-record.md`                                                        | **工程またはイベントの発生時**に読む（入口の SSOT は `SKILL.md` の工程表）                                                                                      |
| `docs`        | `review-prompt.md` / `review-contract.md` / `style.md`                                                                              | 更新判定用 / レビュー委譲の契約 / agent-facing 文書の記法                                                                                                       |
| `rabi-design` | `DESIGN.md`                                                                                                                         | ブランドの仕様。front matter が SSOT                                                                                                                            |

skill 固有の `scripts/`:

| skill         | 実体                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 何をするか                                                                                                                                              |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `conductor`   | `watch.sh` + `watch.test.sh` / `complete-rest-list.sh` + `complete-rest-list.test.sh` / `ensure-integration-ref.test.sh`（共有実体の検査。回すのは `conductor` だけ） / `pr-list.jq` / `comment-fingerprint.jq` / `issue-fingerprint.py` / `cycle-mark.py` + `cycle-mark.test.py` / `project-status.graphql` / `restrict-to-board.awk`                                                                                                                                                                                                                                            | 起床監視・REST 一覧の完全性照合・共有の統合先 ref ensure の検査・checks 抽出・コメント指紋・本文 digest・成果の指紋・台帳クエリ・issues を board へ絞る |
| `docs`        | `audit-skills.sh` / `check-emphasis.ts` / `check-hard-wrap.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 品質パスの機械検査。層の定義 `layers.tsv` を伴う                                                                                                        |
| `resolve`     | `serialize-plan.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 計画コメントの投稿ゲート。marker 抽出と YAML parse と path の round-trip                                                                                |
| `rabi-design` | `gen-css.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `DESIGN.md` の front matter から `assets/` を生成する（`--check` は差分を落とす）                                                                       |
| `pr`          | `sync-and-push.sh` + `sync-and-push.test.sh`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | base への追随と origin の同名への push（素の `git push` を使わせない）                                                                                  |
| 共有          | `shared/roster.toml`（`consult` / `zero-base-loop` の `scripts/` と `conductor/references` から symlink）+ `shared/roster.ts`（`consult` / `zero-base-loop` の `scripts/` と `conductor/src` から symlink）+ `shared/advisors.ts` + `shared/advisors.sh`（`consult` / `zero-base-loop` の `scripts/` から symlink）+ `shared/queue/standalone-line.ts`（`conductor/src` と `resolve/scripts/` から symlink）+ `shared/sync-local-default.sh`（`pr` / `ship` の `scripts/` から symlink） + `shared/ensure-integration-ref.sh`（`conductor` / `merge` の `scripts/` から symlink） | 候補表と実行器・その parse・選出・起動と回収・固定 marker の単独行・ローカル default の ff・統合先 ref の ensure                                        |

skill 固有の `assets/`:

| skill         | 実体              | 何を持つか                         |
| ------------- | ----------------- | ---------------------------------- |
| `rabi-design` | `rabi-tokens.css` | 生成物。`@font-face` と `--rabi-*` |
| `rabi-design` | `rabi-head.html`  | 生成物。webfont の `<link>`        |

## skill を参照する repo 側の実体

skill の外に在り、skill 側からは参照し**ない**。役割は [`../../AGENTS.md`](../../AGENTS.md) の配置方針。

| 実体       | 何を参照するか                                                  |
| ---------- | --------------------------------------------------------------- |
| `../test/` | `bun test` の gate。範囲は [`../../AGENTS.md`](../../AGENTS.md) |

## 追加・変更するとき

手順は [`../AGENTS.md`](../AGENTS.md) の層契約と [`../../AGENTS.md`](../../AGENTS.md) の配置方針。この資料を引き直す条件は `docs` skill が持つ。
